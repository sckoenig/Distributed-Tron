...
| UC-X | `initialize(config : Configuration, modus : GameModus, singleView : boolean, executionService : ExecutorService) : void`                                            | GameManager | Es existiert ein GameManager-Objekt | Game-Objekt ist erstellt und Views sind über ModelState informiert. |  Ein Game wird anhand der Eingabeparameter initialisiert und die Views erden über den ModelState informiert. | - |
| UC-X | `playGame(listenerID : int, playerCount : int) : void`                                            | GameManager | | Das Spielergebnis (Gewinner-Farbe, Ergebnis-Text) wurde gesetzt. | Setzt das Spielergebnis im Game Manager fest, sodass es dem Spieler angezeigt werden kann. | - |
